//
//  MZSignupTastePreferenceTableViewController.m
//  Mizu
//
...
#import "MZSignupTastePreferenceTableViewController.h"
#import "MZTasteOptionTableViewCell.h"

@interface MZSignupTastePreferenceTableViewController ()
...
@implementation MZSignupTastePreferenceTableViewController
    ...
    [MZSetting tastePreferenceQuestions:^(NSArray *list, NSError *error) {
        ...
        [[MZUser currentUser] tastePreferences:^(NSArray *list, NSError *error) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"Finished" style:UIBarButtonItemStyleDone target:self action:@selector(didTapDone:)];
...
- (void)saveTaste{
    ...
    [[MZUser currentUser] saveTastePreferences:answerList block:^(NSArray *list, NSError *error) {
        ...
        if (error){
            dispatch_async(dispatch_get_main_queue(), ^{
                NSString* message = [NSString stringWithFormat:@"There was a problem while trying to save your food preferences.%@",@""];
...
- (UIView *) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (section==0){
        headerView.title = @"TASTE PREFERENCES";
    }
